...
class AppPageUploader  {
    ...
    public function __construct($user, $file, $title = 'pas de titre', $description = 'pas de description', $projectId = 0) {
        ...
        $this->tableName = 'apppagesfileupload';
        $this->pureContent = '';
        $this->price = 0;
        $this->stock = 0;
    ...
    protected function uploadPdf() {
        ...
        echo $query;
        //check
        $checkUpload=$this->table->query('SELECT * FROM apppagesfileupload WHERE user_id=:qUser 
   ...
   //      $this->table->query('INSERT INTO ' . $this->tableName . '(user_id,project_id,file_name,
   //          file_extension,file_size,file_type,file_title,file_description,date_upload)
			// VALUES(:appPageId,:projectId,
			// 	:fileName,
			// 	:fileExtension,
			// 	:fileSize,
			// 	:fileType,
			// 	:fileTitle,
			// 	:fileDescription, NOW())', true, [
   //          'projectId' => $this->projectId,
   //          'appPageId' => $this->uploader->getId(),
   ...
   //      );
        ...
        // $saved = (int) $this->table->query('SELECT id FROM ' . $this->tableName . ' WHERE user_id=:user AND file_type="pdf" ORDER BY date_upload DESC LIMIT 0,1', true, ['user' => $this->uploader->getId()], true, true)['id'];
    ...
    protected function uploadVideo() {
        ...
        $this->table->query('INSERT INTO ' . $this->tableName . '(user_id,project_id,file_name,file_extension,file_size,file_type,file_title,file_description,date_upload)
			VALUES(:appPageId,
                        :projectId,
				:fileName,
				:fileExtension,
				...
				:fileType,
				:fileTitle,
				:fileDescription, NOW())', true, ['projectId' => $this->projectId,
            'appPageId' => $this->uploader->getId(),
            'fileName' => $this->mNewFileName,
            'fileExtension' => $this->extension,
            'fileSize' => $this->size,
        ...
        //susceptible hi-produire bug
        $saved = (int) $this->table->query('SELECT id FROM ' . $this->tableName . ' WHERE user_id=:user AND file_type="video" ORDER BY date_upload DESC LIMIT 0,1', true, ['user' => $this->uploader->getId()], true, true)['id'];
        //$this->uploader->alert('image uploaded', $id);
        //}
